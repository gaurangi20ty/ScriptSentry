"""
ScriptSentry — Kaggle GPU Training (Replay Phase)
===================================================
Self-contained script for training on Kaggle with GPU.
Upload this file + your IAM dataset to Kaggle, then run.

KAGGLE SETUP:
  1. Upload IAM dataset as a Kaggle dataset (or use existing one)
  2. Create a new Kaggle Notebook, set Accelerator → GPU
  3. Add your IAM dataset → it will mount at /kaggle/input/<dataset-name>/
  4. Update DATA_DIR below to match the mount path
  5. Run all cells or: !python kaggle_replay_gpu.py

OUTPUTS (saved to /kaggle/working/results/):
  - replay_acc_matrix.npy        → 5×5 accuracy matrix
  - replay_after_sem{1-5}.pt     → model checkpoints
  - replay_training_log.txt      → full training log
  - All phase plots (.png)

After training, download the /kaggle/working/results/ folder
and copy into your local ScriptSentry/results/ directory.
"""
...
# ================================================================
# CONFIGURATION — Update DATA_DIR for your Kaggle dataset path
# ================================================================
# Common Kaggle IAM dataset paths (update to match yours):
#   /kaggle/input/iam-handwriting-word-database/
#   /kaggle/input/iam-words/
DATA_DIR = "/kaggle/input/iam-handwriting-word-database"

# Training hyperparameters — GPU allows more epochs & bigger buffer
NUM_SEMESTERS      = 5
WRITERS_PER_SEM    = 30
TOTAL_WRITERS      = NUM_SEMESTERS * WRITERS_PER_SEM   # 150
BATCH_SIZE         = 64         # larger batch on GPU
LEARNING_RATE      = 0.001
EPOCHS_PER_SEM     = 30         # 2× more epochs than CPU
BUFFER_SIZE        = 3000       # 2× larger buffer
SAMPLES_PER_CLASS  = 20         # 2× more samples per class in buffer
NUM_WORKERS        = 2          # parallel data loading on Kaggle
...
LOG_PATH = os.path.join(RESULTS, "replay_training_log.txt")
log_file = None
...
def list_dir_tree(data_dir, max_depth=3):
    """Print directory tree to help diagnose path issues."""
...
def find_iam_paths(data_dir):
    """Auto-detect where words/, words.txt, forms.txt are inside data_dir."""
    words_txt = None
    words_dir = None

    # Also look for any directory containing .png files (fallback for flat layouts)
    png_dirs = set()
    ...
    # Fallback: if no words/ dir found but we found PNGs, use the common parent
    if words_dir is None and png_dirs:
        # Find the shallowest directory containing PNGs
...
def build_student_dataset(data_dir):
    ...
    if words_dir is None:
        raise FileNotFoundError(
            ...
            f"Try: DATA_DIR = '/kaggle/input/<your-dataset-name>'"
        )
    if words_txt is None:
        raise FileNotFoundError(
    ...
    if forms_txt and os.path.isfile(forms_txt):
        ...
        log("  ℹ forms.txt not found — using form IDs as student identifiers")
...
def load_all_semesters(data_dir, num_semesters, writers_per_sem, batch_size):
    """Load all semester data loaders."""
    log("\n" + "=" * 60)
    ...
    log(f"  IDs: {selected[:3]} … {selected[-3:]}\n")
...
def plot_forgetting(acc_matrix, path):
    ...
    ax.set_title("Replay — Knowledge Retention Over Time", fontsize=15, fontweight="bold")
...
def plot_all_semesters(acc_matrix, path):
    ...
    ax.set_title("Replay — Peak vs Final Accuracy", fontsize=15, fontweight="bold")
...
# ================================================================
# MAIN TRAINING LOOP — REPLAY
# ================================================================
def train_replay():
    log("\n" + "=" * 60)
    log("  ScriptSentry — GPU Replay Training")
    log(f"  Device: {DEVICE}")
    ...
    log(f"  vs Random chance       : {avg_final / (100/150):.0f}×")
    ...
    plot_heatmap(acc_matrix,
                 f"GPU Replay — Accuracy Matrix\n"
                 f"(Epochs={EPOCHS_PER_SEM}, Buffer={BUFFER_SIZE}, "
                 f"Samples/class={SAMPLES_PER_CLASS})",
                 os.path.join(RESULTS, "replay_gpu_heatmap.png"))
    plot_forgetting(acc_matrix,
                    os.path.join(RESULTS, "replay_gpu_forgetting.png"))
    plot_all_semesters(acc_matrix,
                       os.path.join(RESULTS, "replay_gpu_all_semesters.png"))

    # ── Save final model ──
    final_path = os.path.join(CKPTS, "replay_gpu_final.pt")
    torch.save({
        "model_state_dict": model.state_dict(),
    ...
    log(f"\n{'=' * 60}")
    log(f"  ✅ GPU Replay training complete!")
    log(f"  Download /kaggle/working/results/ and /kaggle/working/checkpoints/")
    log(f"  Copy into your local ScriptSentry project.")
    log(f"{'=' * 60}\n")
